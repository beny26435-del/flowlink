...
import { flowLinkV3Abi } from "../src/flowlink-v3/abi";
import {
  createFlowLinkV3PublicClient,
  createFlowLinkV3WalletClient,
  createGroupLink,
  createInvoiceLink,
  createPaymentLink,
  createUnlockLink,
...
const publicClient = createFlowLinkV3PublicClient(rpcUrl);
const walletClient = createFlowLinkV3WalletClient(privateKey, rpcUrl);
const config = {
  contractAddress,
  rpcUrl,
  publicClient,
...
console.log("Pay/contribute/refund are disabled in this smoke script.");

const profileTx = await upsertProfile(config, {
  username,
  displayName: "FlowLink Smoke",
  bio: "Created by scripts/smoke-v3.ts on Arc Testnet.",
  avatarUrl: "",
});
...
const created = [
  await createAndRead(
    ...
    createPaymentLink(config, {
      ...
      description: `Created by scripts/smoke-v3.ts at ${new Date().toISOString()}`,
      slug: paymentSlug,
      listed: true,
    }),
  ),
...
function readContractAddress(): Address {
  const value = process.env.FLOWLINK_V3_CONTRACT_ADDRESS ?? process.env.NEXT_PUBLIC_FLOWLINK_V3_CONTRACT_ADDRESS;
  if (!value || !isAddress(value)) {
    throw new Error("Set FLOWLINK_V3_CONTRACT_ADDRESS or NEXT_PUBLIC_FLOWLINK_V3_CONTRACT_ADDRESS to the deployed FlowLink address.");
  }
